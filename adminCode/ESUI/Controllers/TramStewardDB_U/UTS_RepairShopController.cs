...
{
    //[Export]
    public class UTS_RepairShopController : BaseController
    {
     ...
     //   public ITS_RepairShopDao OPBiz { get; set; }
       // public TS_RepairShopBiz OPBiz = new TS_RepairShopBiz();
        ...
        public JsonResult EditInfo(TS_RepairShop DitModle)
        {
            ...
            {
                DitModle.WhereExpression = TS_RepairShopSet.Id.Equal(DitModle.Id);
                //DitModle.ChangedMap.Remove("doctorid");//移除主键值

                var mqlO = TS_RepairShopSet.SelectAll().Where(TS_RepairShopSet.Id.Equal(DitModle.Id));
                TS_RepairShop modelOld = OPBiz.GetEntity(mqlO);//旧数据

...
{
    //[Export]
    public class UTS_ServiceController : BaseController
    {
     ...
     //   public ITS_ServiceDao OPBiz { get; set; }
       // public TS_ServiceBiz OPBiz = new TS_ServiceBiz();
        ...
        public JsonResult EditInfo(TS_Service DitModle)
        {
            ...
            {
                DitModle.WhereExpression = TS_ServiceSet.Id.Equal(DitModle.Id);
                //DitModle.ChangedMap.Remove("doctorid");//移除主键值

                var mqlO = TS_ServiceSet.SelectAll().Where(TS_ServiceSet.Id.Equal(DitModle.Id));
                TS_Service modelOld = OPBiz.GetEntity(mqlO);//旧数据
